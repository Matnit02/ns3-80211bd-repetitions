...
OfdmPhy::GetOfdmPpduFormatsTable()
{
    static const PhyEntity::PpduFormats formats{
        {WIFI_PREAMBLE_LONG,
...
const PhyEntity::ModulationLookupTable&
OfdmPhy::GetOfdmModulationLookupTable()
{
    static const PhyEntity::ModulationLookupTable table{{
        {"OfdmRate32_5MbpsBW10MHz", {WIFI_CODE_RATE_5_6, 64}},   // 64-QAM 5/6
        {"OfdmRate39MbpsBW10MHz", {WIFI_CODE_RATE_3_4, 256}},    // 256-QAM 3/4
...
GetOfdmRatesBpsList()
{
    static const std::map<MHz_u, std::array<uint64_t, 8>> s_ofdmRatesBpsList{{
        {MHz_u{20}, {6000000, 9000000, 12000000, 18000000, 24000000, 36000000, 48000000, 54000000}},
        {MHz_u{10}, {3000000, 4500000, 6000000, 9000000, 12000000, 18000000, 24000000, 27000000}},
        {MHz_u{5}, {1500000, 2250000, 3000000, 4500000, 6000000, 9000000, 12000000, 13500000}},
    }};
...
OfdmPhy::OfdmPhy(OfdmPhyVariant variant /* = OFDM_PHY_DEFAULT */, bool buildModeList /* = true */)
{
    ...
    if (buildModeList)
    {
        ...
        case OFDM_PHY_10_MHZ:
            ...
            {
            WifiMode mode = GetOfdmRate (32500000, MHz_u{10});
            NS_LOG_LOGIC ("Add " << mode << " to list");
            m_modeList.emplace_back (mode);

            mode = GetOfdmRate (39000000, MHz_u{10});
            NS_LOG_LOGIC ("Add " << mode << " to list");
            m_modeList.emplace_back (mode);
            }
            break;
...
{
    return GetOfdmPpduFormatsTable();
}
...
OfdmPhy::InitializeModes()
{
    ...
    GetOfdmRate32_5MbpsBW10MHz();
    GetOfdmRate39MbpsBW10MHz();
}
...
OfdmPhy::GetOfdmRate(uint64_t rate, MHz_u bw)
{
    ...
    case 10:
        ...
        case 32500000: return GetOfdmRate32_5MbpsBW10MHz();
        case 39000000: return GetOfdmRate39MbpsBW10MHz();
        default:
...
GET_OFDM_MODE (OfdmRate32_5MbpsBW10MHz, false);  // new
GET_OFDM_MODE (OfdmRate39MbpsBW10MHz,   false);  // new
...
OfdmPhy::CreateOfdmMode(std::string uniqueName, bool isMandatory)
{
    ...
    const auto& table = GetOfdmModulationLookupTable();
    const auto it = table.find(uniqueName);
    NS_ASSERT_MSG(it != table.end(),
...
{
    return GetOfdmModulationLookupTable().at(name).first;
}

uint16_t
OfdmPhy::GetConstellationSize(const std::string& name)
{
    return GetOfdmModulationLookupTable().at(name).second;
}
...
{
    // Special cases for 802.11bd-like 10 MHz rates
    if (name == "OfdmRate32_5MbpsBW10MHz" && channelWidth == MHz_u{10})
    {
        return 32500000; // 32.5 Mbps
    }
    if (name == "OfdmRate39MbpsBW10MHz" && channelWidth == MHz_u{10})
    {
        return 39000000; // 39 Mbps
    }

    // Default 802.11a/p behaviour
    WifiCodeRate codeRate = GetCodeRate (name);
