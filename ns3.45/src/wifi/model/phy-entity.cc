...
PhyEntity::CalculatePhyPreambleAndHeaderDuration(const WifiTxVector& txVector) const
{
    Time duration;
    const auto& ppduFormats = GetPpduFormats();
    const auto itPpdu = ppduFormats.find(txVector.GetPreambleType());
    if (itPpdu != ppduFormats.end())
    {
        for (const auto field : itPpdu->second)
        {
            if (field == WIFI_PPDU_FIELD_DATA)
            {
                break;
            }
            duration += GetDuration(field, txVector);
        }
        return duration;
    }
    // Fallback for legacy/unknown preambles keeps previous enum-based behavior.
