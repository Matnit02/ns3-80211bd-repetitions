                                          ...
                                          WifiPpduField header) const
{
    ...
    auto niIt = nis->find(band)->second;
    auto modulation = event->GetPpdu()->GetTxVector().GetModulationClass();
    if (event->GetPpdu()->GetTxVector().GetPreambleType() == WIFI_PREAMBLE_BD)
    {
        modulation = WIFI_MOD_CLASS_BD;
    }
    auto phyEntity = WifiPhy::GetStaticPhyEntity(modulation);
