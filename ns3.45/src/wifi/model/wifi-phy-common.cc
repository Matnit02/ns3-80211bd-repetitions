...
namespace
{
bool g_isBdStandardSelected{false};
} // namespace

void
SetBdStandardSelected(bool enabled)
{
    g_isBdStandardSelected = enabled;
}

bool
IsBdStandardSelected()
{
    return g_isBdStandardSelected;
}
...
GetPreambleForTransmission(WifiModulationClass modulation, bool useShortPreamble /* = false */)
{
    ...
    else if (modulation == WIFI_MOD_CLASS_BD)
    {
        return WIFI_PREAMBLE_BD;
    }
    else if (modulation == WIFI_MOD_CLASS_OFDM && IsBdStandardSelected())
    {
        // Reuse OFDM mode names in 11bd while still selecting the BD PPDU format.
        return WIFI_PREAMBLE_BD;
    }
    else
    {
...
GetModulationClassForPreamble(WifiPreamble preamble)
{
    ...
    case WIFI_PREAMBLE_BD:
        return WIFI_MOD_CLASS_BD;
    default:
                                      ...
                                      WifiModulationClass modClassAnswer)
{
    ...
    case WIFI_MOD_CLASS_BD:
        return (modClassAnswer == WIFI_MOD_CLASS_BD);
...
GetModulationClassForStandard(WifiStandard standard)
{
    ...
    case WIFI_STANDARD_80211p:
        ...
        break;
    // Added WIFI_STANDARD_80211bd
    case WIFI_STANDARD_80211bd:
        modulationClass = WIFI_MOD_CLASS_BD;
        break;
