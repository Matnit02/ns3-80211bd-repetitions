...
{
WifiModulationClass
GetTxModulationForPhyEntity(const WifiTxVector& txVector)
{
    auto modClass = txVector.GetModulationClass();
    if (txVector.GetPreambleType() == WIFI_PREAMBLE_BD)
    {
        modClass = WIFI_MOD_CLASS_BD;
    }
    return modClass;
}
} // namespace
...
{
    NS_ABORT_IF(!ppdu);
    auto modulation = ppdu->GetModulation();
    if (ppdu->GetTxVector().GetPreambleType() == WIFI_PREAMBLE_BD)
    {
        modulation = WIFI_MOD_CLASS_BD;
    }
...
void
WifiPhy::Configure80211bd()
{
    NS_LOG_FUNCTION(this);
    if (GetChannelWidth() == MHz_u{10})
    {
        AddPhyEntity(WIFI_MOD_CLASS_BD, Create<ns3::BdPhy>(OFDM_PHY_10_MHZ));
        AddPhyEntity(WIFI_MOD_CLASS_OFDM, Create<OfdmPhy>(OFDM_PHY_10_MHZ));

        // Reuse 10 MHz OFDM timing defaults
        SetSifs(OFDM_SIFS_TIME_10MHZ);
        SetSlot(OFDM_SLOT_TIME_10MHZ);
        SetPifs(GetSifs() + GetSlot());
    }
    ...
    {
        NS_FATAL_ERROR("802.11bd configured with a wrong channel width!");
    }
}
...
WifiPhy::ConfigureStandard(WifiStandard standard)
{
    ...
    // Track bd selection for preamble choice
    SetBdStandardSelected(m_standard == WIFI_STANDARD_80211bd);
...
WifiPhy::GetPayloadDuration(uint32_t size,
                            ...
                            uint16_t staId)
{
    return GetStaticPhyEntity(GetTxModulationForPhyEntity(txVector))
...
WifiPhy::Send(const WifiConstPsduMap& psdus, const WifiTxVector& txVector)
{
    ...
    auto ppdu = GetPhyEntity(GetTxModulationForPhyEntity(txVector))
