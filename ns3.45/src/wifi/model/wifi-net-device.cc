...
{
    if (m_standard == WIFI_STANDARD_80211bd)
    {
        return nullptr; // 802.11bd is non-HT (10 MHz OFDM)
    }
...
WifiNetDevice::GetVhtConfiguration() const
{
    if (m_standard == WIFI_STANDARD_80211bd)
    {
        return nullptr; // bd devices do not advertise VHT capabilities
    }
...
WifiNetDevice::GetHeConfiguration() const
{
    if (m_standard == WIFI_STANDARD_80211bd)
    {
        return nullptr; // bd devices are non-HE
    }
...
WifiNetDevice::GetEhtConfiguration() const
{
    if (m_standard == WIFI_STANDARD_80211bd)
    {
        return nullptr; // bd devices are non-EHT
    }
