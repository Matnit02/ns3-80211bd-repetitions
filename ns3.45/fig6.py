...
def build_stats(df: pd.DataFrame, target_densities, min_density) -> pd.DataFrame:
    ...
    return stats.sort_values(["mcs", "density"])
...
def plot_improvement_line(
    ...
    print("Calculating gain between rep 1 and rep 3 (LDPC vs No-LDPC)...")

    ldpc_stats = build_stats(ldpc_df, target_densities, min_density)
    no_ldpc_stats = build_stats(no_ldpc_df, target_densities, min_density)

    if ldpc_stats.empty:
        print("No LDPC gain data computed.")
        return
    if no_ldpc_stats.empty:
        print("No No-LDPC gain data computed.")
        return


    mcs_common = sorted(
        set(ldpc_stats["mcs"].unique()).intersection(no_ldpc_stats["mcs"].unique()),
        key=lambda m: (MCS_ORDER.get(m, 999), m),
    )
    if not mcs_common:
        print("No overlapping MCS between LDPC and No-LDPC.")
        return

    ldpc_stats = ldpc_stats[ldpc_stats["mcs"].isin(mcs_common)].copy()
    no_ldpc_stats = no_ldpc_stats[no_ldpc_stats["mcs"].isin(mcs_common)].copy()
    ...
    colors = cmap(np.linspace(0.1, 0.9, max(len(mcs_common), 1)))
    markers = [
        "o",
        "s",
        "^",
    ...
    mcs_to_style = {
        ...
        for i, mcs in enumerate(mcs_common)
    }

    for mcs in mcs_common:
        ldpc_sub = ldpc_stats[ldpc_stats["mcs"] == mcs].sort_values("density")
        no_ldpc_sub = no_ldpc_stats[no_ldpc_stats["mcs"] == mcs].sort_values("density")
    ...
    xmin = float(min(ldpc_stats["density"].min(), no_ldpc_stats["density"].min()))
    xmax = float(max(ldpc_stats["density"].max(), no_ldpc_stats["density"].max()))
    spread = max(xmax - xmin, 1.0)
    pad = 0.05 * spread
    ax.set_xlim(xmin - pad, xmax + pad)
    plt.xlabel("Vehicle Density [veh./km]")
    plt.ylabel("Mean PRR Gain (Rep 3 − Rep 1) ± 99% Confidence Band")
...
def main():
    ...
    ap.add_argument(
        "--out-plot",
        default="line_improvement_vs_density_ldpc_vs_noldpc.pdf",
    ...
    plot_improvement_line(ldpc_data, no_ldpc_data, args.densities, args.min_density, args.out_plot)
